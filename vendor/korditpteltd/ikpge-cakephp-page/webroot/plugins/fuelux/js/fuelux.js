...
( function( factory ) {
	...
	( function( $ ) {
		...
		var Wizard = function( element, options ) {
			...
			//Hack
			this.$sglPrevBtn = this.$element.find( 'button.btn-prev' ).first();
			this.$sglNextBtn = this.$element.find( 'button.btn-next' ).first();
			...
			kids = this.$sglNextBtn.children().detach();
			this.nextText = $.trim( this.$sglNextBtn.text() );
		...
		Wizard.prototype = {
			...
			setState: function() {
				...
				if ( last ) {
					...
					var kids = this.$sglNextBtn.children().detach();
