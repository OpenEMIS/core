...
INNER JOIN (
	SELECT intStud1.id, intStud1.created, intStud1.student_id
	...
	ON stud.student_id = intStud2.student_id AND intStud2.created < stud.created
...
WHERE institution_students.id IN 
(SELECT tmpTable.id FROM (
	SELECT intStud2.id
	FROM institution_students intStud2
	INNER JOIN (
		SELECT intStud1.id, intStud1.created, intStud1.student_id
