...
******************************************************************************************************************/
	public function beforeSave(Event $event, Entity $entity) {
		if (!empty($entity->id) && $entity->getDirty() && $this->_table->trackActivity && isset($this->_table->fields)) { // edit operation
			$model = $this->_table;
		    ...
		    if (method_exists($entity, 'visibleProperties')) {
								...
								foreach ($allData as $allDataKey=>$allDataValue) {
									...
									if (is_object($relatedModel)) {
										...
										if (substr_count($field, '_id')>0) {
											...
											Log::write('debug', $field." is not defined in belongsTo ".$model->getAlias()." @ TrackActivityBehaviour beforeSave()");
										}
										$obj[$allDataKey.'_value'] = ($allDataValue) ? $allDataValue : ' ';
									}
								}
