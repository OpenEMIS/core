...
class AssociationExcelBehavior extends Behavior
{
    use EventTrait;
    ...
    public function generate($settings = [])
    {
        ...
        $condition = [];
        $this->AcademicPeriods = TableRegistry::getTableLocator()->get('AcademicPeriod.AcademicPeriods');//POCOR-8556
        ...
        foreach ($sheets as $sheet) {
			...
			$query = $Query
				->select([
					...
					'association_name' => 'InstitutionAssociations.name',
					...
					'openEMIS_ID' => 'AssociationStudents.openemis_no',
					'student_name' => $Query->func()->concat([
						'AssociationStudents.first_name' => 'literal',
						" ",
						'AssociationStudents.last_name' => 'literal'
					]),
					'gender' => 'Genders.name',
				...
				->contain([
					...
					// 'AssociationStaff' => [
					// 	'fields' => [
					// 		'AssociationStaff.openemis_no',
					// 		'AssociationStaff.first_name',
					// 		'AssociationStaff.middle_name',
					// 		'AssociationStaff.third_name',
					// 		'AssociationStaff.last_name'
					// 	]
					// ]
				])
				->leftJoin(
				['InstitutionAssociationStudent' => 'institution_association_student'],
				[
				...
				[
					'StudentStatuses.id = InstitutionAssociationStudent.student_status_id'
				]
				)
				->leftJoin(
				['InstitutionAssociationStaff' => 'institution_association_staff'],
				[
					'InstitutionAssociationStaff.institution_association_id = '. $InstitutionAssociations->aliasField('id')
				]
				)
				->leftJoin(
				...
				->leftJoin(
				['AssociationStudents' => 'security_users'],
				[
					'AssociationStudents.id = '. $InstitutionStudents->aliasField('security_user_id')
				]
				)
				->leftJoin(
				['Genders' => 'genders'],
				[
					'AssociationStudents.gender_id = Genders.id'
				]
				)
                //POCOR-5852 starts
                ->where($conditions)
