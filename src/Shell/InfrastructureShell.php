...
class RoomShell extends Shell
{
    ...
    private function copyProcess($copyFrom, $copyTo)
    {
        ...
        if ($this->checkIfCanCopy('Institution.InstitutionLands', $copyTo)) {
            ...
            if (!empty($contain)) {
                $containCount = count(split('.', $contain));
                $containQuery = [];
                switch ($containCount) {
                    case 1:
                        $containQuery = [
        ...
        while ($query->page($page, $limit)->count() > 0) {
            ...
            foreach ($executedQuery->toArray() as $land) {
                ...
                if ($landRecord->offsetExists('institution_buildings')) {
                    $saveOptions = ['associated' => 'InstitutionBuildings'];
                    foreach ($landRecord->institution_buildings as &$building) {
