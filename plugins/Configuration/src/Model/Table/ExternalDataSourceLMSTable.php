...
class ExternalDataSourceLMSTable extends ControllerActionTable
{
    ...
    {
        $query->where([$this->aliasField('type') => 'External Data Source - LMS',
    ...
    public function editAfterSave(Event $event, Entity $entity, ArrayObject $patchOption, ArrayObject $extra)
    {
        ...
        if (!empty($errors) || empty($entity->base_url)|| empty($entity->api_token)|| empty($entity->enable_user_creation)|| empty($entity->status)) {
