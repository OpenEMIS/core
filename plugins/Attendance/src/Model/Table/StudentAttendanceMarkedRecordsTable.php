...
class StudentAttendanceMarkedRecordsTable extends AppTable
{
    ...
    public function findNoScheduledClass(Query $query, array $options) 
    {
        ...
        $query->where([$this->aliasField('institution_id') => $institutionId, $this->aliasField('academic_period_id') => $academicPeriodId,$this->aliasField('institution_class_id') => $institutionClassId,$this->aliasField('education_grade_id') => $educationGradeId]); //POCOR-8372 taking log time in load
        $row = [];
