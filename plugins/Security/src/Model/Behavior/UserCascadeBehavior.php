...
class UserCascadeBehavior extends Behavior {
	...
	private function cleanUserRecords($userId) {
		...
		foreach ($tables as $key => $table) {
			...
			if ($this->_table->startsWith($table, 'zz_')) { // to exclude all z_ prefix tables
				continue;
			}
			if ($this->_table->startsWith($table, 'inserted_records')) { // to exclude all z_ prefix tables
				continue;
			}
			try {
				if (!in_array($table, $excludes)) {
                    $tableObj = self::getDynamicTableInstance($table);
					foreach ($fields as $field) {
                        $column = $tableObj->getSchema()->getColumn($field);
                        if ($column) {
				...
				if (!in_array($table, $excludes)) {
					foreach ($fields as $field) {
						if (in_array($field, $columns)) {
							echo 'DELETE FROM ' . $tableObj->getTable() . ' WHERE NOT EXISTS (SELECT 1 FROM security_users WHERE security_users.id = ' . $table . '.' . $field . ');<br>';
						}
					}
				}
			} catch (Exception $ex) {
