 ...
 * @author lakshman Jangid <lakshman.jangid@mail.valuecoders.com>
 */
...
class StaffRequirementsTable extends AppTable
{
    ...
    public function onExcelBeforeQuery(Event $event, ArrayObject $settings, Query $query)
    {
        ...
        // get operational academic periods
        $oapt2 = $this->find()
            ->select([
        ...
        //previous_current_join_3 //left_join_t_2
        ...
        $studentQuery = $this->find()
            ->select([
                ...
                'education_subjects_id' => 'education_subjects.id',
                ...
                'Students_per_Teacher' => '@Students_per_Teacher := ' . $studentPerTeacherRatio,
                'Lower_Tolerance' => '@Lower_Tolerance := ' . $lowerTolerance,
                'Upper_Tolerance' => '@Upper_Tolerance := ' . $upperTolerance,
            ...
            ->innerJoin(['institutions' => $institution->table()], [
                'institutions.id = student_subjects.institution_id'
            ])
            ->innerJoin(['area_layer_one' => $area->table()], [
                'area_layer_one.id = institutions.area_id'
            ])
        ...
        $query
            ->select([
                ...
                'historical_t4_staff' => 'ROUND(IF(IFNULL(staff_query.2018_staff, 0) < 1, 0, IFNULL(staff_query.2018_staff, 0)), 0)',
                'historical_t3_staff' => 'ROUND(IF(IFNULL(staff_query.2019_staff, 0) < 1, 0, IFNULL(staff_query.2019_staff, 0)), 0)',
                'historical_t2_staff' => 'ROUND(IF(IFNULL(staff_query.2020_staff, 0) < 1, 0, IFNULL(staff_query.2020_staff, 0)), 0)',
                'historical_t1_staff' => 'ROUND(IF(IFNULL(staff_query.2021_staff, 0) < 1, 0, IFNULL(staff_query.2021_staff, 0)), 0)',
            ...
            ])->leftJoin(['StaffRequirements' => $this->table()], [
                'StaffRequirements.id = staff_query.institution_id'
            ]);
