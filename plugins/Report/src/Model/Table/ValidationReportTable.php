...
class ValidationReportTable extends AppTable  {
    ...
    public function onExcelBeforeQuery(EventInterface $event, ArrayObject $settings, Query $query) 
    {
        ...
        foreach ($AreaData as $row) {
            $childArea[] = (int)$row['id'];
        }

        // level 2
        $childAreaMain = [];
        if (!empty($childArea)) {
            $AreaDatas = $AreaT->find()
                ->select(['id'])
                ->where(['parent_id IN' => $childArea])
                ->enableHydration(false)
                ->toArray();

            foreach ($AreaDatas as $row) {
                $childAreaMain[] = (int)$row['id'];
            }
        }
        ...
        if (!empty($childAreaMain)) {
            $AreaDatas = $AreaT->find()
                ->select(['id'])
                ->where(['parent_id IN' => $childAreaMain])
                ->enableHydration(false)
                ->toArray();

            foreach ($AreaDatas as $row) {
                $childArea3[] = (int)$row['id'];
            }
        }
        ...
        if (!empty($childArea3)) {
            $AreaDatas = $AreaT->find()
                ->select(['id'])
                ->where(['parent_id IN' => $childArea3])
                ->enableHydration(false)
                ->toArray();

            foreach ($AreaDatas as $row) {
                $childArea4[] = (int)$row['id'];
            }
        }

        // ===============================
        // FINAL AREA IDS (BULLET PROOF)
        // ===============================

        $mergeArr = array_merge($childArea, $childAreaMain, $childArea3, $childArea4);
        ...
        // sanitize
        $mergeArr = array_map('intval', $mergeArr);
        $mergeArr = array_filter($mergeArr);     // remove 0/null
        $mergeArr = array_unique($mergeArr);

        $finalIds = $mergeArr;   // ✅ SAFE ARRAY

        // ===============================
        // BUILD WHERE + whereClause SAFE
        // ===============================
        ...
        if (!empty($whereConditions)) {
            ...
            $whereClause = '';   // ✅ NEVER produces IN ()
        }

        // ===============================
        // JOINS (UNCHANGED, NOW SAFE)
        // ===============================
        ...
        $join['grades'] = [
            ...
            'table' => "
                (
                    SELECT aggregated_grades.institution_id
                        , lowest_stage.name  AS lowest_grade
                        , highest_stage.name AS highest_grade
                    FROM
                    (
