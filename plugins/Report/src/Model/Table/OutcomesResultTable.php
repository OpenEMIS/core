...
class OutcomesResultTable extends AppTable
{
    ...
    public function onExcelBeforeQuery(Event $event, ArrayObject $settings, $query)
    {
        ...
        $periodList = $OutcomePeriodsTable
            ->find()
            ->where([
                $OutcomePeriodsTable->aliasField('academic_period_id IS') => $academicPeriodId,
            ])
            ->extract('id')
            ->toArray();
    ...
    public function onExcelUpdateFields(Event $event, ArrayObject $settings, $fields)
    {
       ...
       $newFields[] = [
        ...
        // Step 2: Add fields in order: all criteria, then comment + final result
