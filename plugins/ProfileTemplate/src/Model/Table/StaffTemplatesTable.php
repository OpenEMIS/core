...
class StaffTemplatesTable extends ControllerActionTable
{
    ...
    // Note: previously this method shifted generate_start_date/generate_end_date forward by
    // '+2 day' before saving, with no corresponding adjustment anywhere on read/display. That made
    // every saved date land 2 days after what the user actually picked (e.g. selecting July 1
    // saved as July 3). beforeMarshal() below already normalizes the submitted value correctly, so
            ...
            if (is_string($rawValue) && !preg_match('/^\d{4}-\d{2}-\d{2}/', $rawValue)) {
                ...
                try {
                    ...
                    try {
                        ...
                        \Cake\Log\Log::warning("StaffTemplatesTable: Invalid date '{$rawValue}' for field '{$field}' with format '{$systemDateFormat}'");
                    }
                }
