...
class StaffProfilesTable extends ControllerActionTable
{
    ...
    public function beforeAction(EventInterface $event, ArrayObject $extra)
    {
        ...
        // Start POCOR-5188
		$is_manual_exist = $this->getManualUrl('Administration','Staff','Profiles');
    ...
    public function onGetStaffName(EventInterface $event, Entity $entity)
    {
        $Users = TableRegistry::getTableLocator()->get('User.Users');
        $result = $Users
            ->find()
    ...
    */
    ...
    private function addReportCardsToProcesses($institutionId, $academicPeriodId, $reportCardId, $staffId = null)
    {
        ...
        $StaffReportCardProcesses = TableRegistry::getTableLocator()->get('ReportCard.StaffReportCardProcesses');
        $staffTable = TableRegistry::getTableLocator()->get('Institution.InstitutionStaff');
        $where = [];
    ...
    {
        $SystemProcesses = TableRegistry::getTableLocator()->get('SystemProcesses');
        $runningProcess = $SystemProcesses->getRunningProcesses($this->StaffReportCardEmailProcesses->getRegistryAlias());
        ...
        if (count($runningProcess) <= self::MAX_PROCESSES) {
            ...
            $pid = '';
            $processModel = $this->StaffReportCardEmailProcesses->getRegistryAlias();
            $eventName = '';
            $passArray = [
                'institution_id' => $institutionId,
                'staff_profile_template_id' => $reportCardId
