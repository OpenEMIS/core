...
function checkNavToggle(isRTL) {
	...
	$(rtlClassName + '#main-splitter').jqxSplitter({ panels: panels });  
...
function storePanelsState(refObject){
	var lastPanelState = [];
	lastPanelState.push({size: refObject[0].size, min: refObject[0].min, collapsible: refObject[0].collapsible});
	lastPanelState.push({size: refObject[1].size, min: refObject[1].min, collapsible: refObject[1].collapsible});
...
function getDefaultPanelSettings(isRTL){
	...
	if(isRTL){  /*id is RTL load RTL settings*/
		var paneWidth = $(window).width() - navWidth;
		 refObject = {
			type: 'RTL',
			data: [{ min: '60%' ,size: paneWidth, collapsible: false }, {  min: 100 , size: navWidth, collapsible: true }]
		};
	}
	else{
		 refObject = {
			type: 'LTR',
			data: [{ size: navWidth, min: 100 , collapsible: true }, { min: '60%', size: '90%', collapsible: false }]
		};
	}
