...
//            'History' => ['className' => 'User.UserActivities', 'actions' => ['index']], //POCOR-7485 cakephp4 use as a function
     ...
     *
     * URL is generated by ImportLink behavior as /Student/Students/ImportCompetencyResults/...
     * while the actual import table class lives in Institution plugin.
     */
    public function ImportCompetencyResults()
    {
        $pass = (array)$this->request->getParam('pass');
        $subAction = $pass[0] ?? 'add';
        $encoded = $pass[1] ?? null;
        $url = [
            'plugin' => 'Institution',
            'controller' => 'Institutions',
            'action' => 'ImportCompetencyResults',
            0 => $subAction,
        ];
        if ($encoded !== null) {
            $url[1] = $encoded;
...
//        $this->log($model, 'debug');
    ...
    //POCOR-9590: PascalCase matches URL convention (StudentGpa, StudentBehaviours, etc.)
