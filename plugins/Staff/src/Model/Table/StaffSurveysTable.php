...
use Cake\ORM\Table;
class StaffSurveysTable extends ControllerActionTable
{
    // Default Status
    const EXPIRED = -1;
    ...
    public function initialize(array $config)
    {
        ...
        $this->addBehavior('CustomField.Record', [
            ...
            'recordKey' => 'institution_staff_survey_id',
            'fieldValueClass' => ['className' => 'Staff.StaffSurveyAnswers', 'foreignKey' => 'institution_staff_survey_id', 'dependent' => true, 'cascadeCallbacks' => true],
            'tableCellClass' => ['className' => 'Staff.StaffSurveyTableCells', 'foreignKey' => 'institution_staff_survey_id', 'dependent' => true, 'cascadeCallbacks' => true, 'saveStrategy' => 'replace']
        ]);
    ...
    public function beforeAction(Event $event, ArrayObject $extra)
    {
        ...
        $indexElements = [
            ['name' => 'staff.staffSurveys/controls', 'data' => [], 'options' => [], 'order' => 1]
        ];
        ...
        $staffId = $this->Session->read('staff.staffs.id');

        // Build Survey Records
        $currentAction = $this->action;
        if ($currentAction == 'index') {
            // Disabled auto-insert New Survey for staff until there is a better solution
            // $this->_buildSurveyRecords($staffId);
        }
        // End
        ...
        $this->advancedSelectOptions($periodOptions, $selectedPeriod, [
            ...
            'callable' => function ($id) use ($institutionId, $staffId) {
                return $this
                    ->find()
                    ->where([
    ...
    private function setupTabElements($entity = null)
    {
        ...
        if (!is_null($entity)) {
            $tabElements['staffSurveys']['url'][0] = 'view';
            $tabElements['staffSurveys']['url'][1] = $this->paramsEncode(['id' => $entity->id]);
        }
    ...
    /* Disabled auto-insert New Survey for staff until there is a better solution
    public function _buildSurveyRecords($staffId=null, $institutionId=null) {
        $session = $this->controller->request->session();
        if ($session->check('Institution.Institutions.id')) {
            $institutionId = $session->read('Institution.Institutions.id');
        }

        if (!is_null($staffId) && !is_null($institutionId)) {
            $surveyForms = $this->getForms();
            ...
            // Update all New Survey to Expired by Institution ID and staff ID
    ...
    */
    ...
    public function _redirect($institutionId = null, $staffId = null, $periodId = 0, $formId = 0)
    {
        $currentAction = $this->action;
