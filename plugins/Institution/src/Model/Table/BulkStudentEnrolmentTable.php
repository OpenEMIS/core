...
class BulkStudentEnrolmentTable extends ControllerActionTable
{
    private $_modelAlias = 'Institution.StudentEnrolment';
    ...
    public function initialize(array $config): void
    {
        ...
        $this->hasMany('StudentEnrolment', ['className' => 'Institution.StudentEnrolment', 'foreignKey' => 'status_id', 'dependent' => true, 'cascadeCallbacks' => true]);
    ...
    public function onGetBreadcrumb(EventInterface $event, ServerRequest $request, Component $Navigation, $persona=false)
    {
        ...
        $url = ['plugin' => 'Institution', 'controller' => 'Institutions', 'action' => 'StudentEnrolment', 'index',$encodedQueryString];
        $Navigation->substituteCrumb('Bulk Student Enrolment', 'Student Enrolment', $url);
        $Navigation->addCrumb('Bulk Student Enrolment');
    }
    ...
    public function editBeforeQuery(EventInterface $event, Query $query, ArrayObject $extra)
    {
        ...
        $query->contain([
            ...
            'StudentEnrolment'=> function ($q) use ($superAdmin, $userId, $institutionId) {
                $q->where(['StudentEnrolment.institution_id' => $institutionId])
                    ->contain(['Users', 'Assignees', 'AcademicPeriods', 'EducationGrades', 'InstitutionClasses', 'Statuses']);
                if ($superAdmin) {
                    return $q;
                } else {
                    return $q->where(['StudentEnrolment.assignee_id'=> $userId]);
                }
            },
            'Workflows.WorkflowModels'
        ])
    ...
    {
        /* gets all the workflow_steps in which the workflow model belongs to StudentEnrolmentTable & returns a list of key-value pair for populating the dropdown. The dropdown contains statuses which have next step(action) */
         switch ($this->action) {
            case 'edit':
                $attr['type'] = 'select';
                $attr['select'] = false;
    ...
    public function onUpdateFieldBulkStudentEnrolment(EventInterface $event, array $attr, $action, ServerRequest $request)
    {
        switch ($this->action) {
            case 'edit':
                $entity = $attr['entity'];
                $students = $entity->student_enrolment;
                break;
            case 'reconfirm':
                $students  = $this->_currentData->student_enrolment;
        ...
        $attr['type'] = 'element';
        $attr['element'] = 'Institution.BulkStudentEnrolment/students';
        $attr['data'] = $students;
        return $attr;
    }
    ...
    public function reconfirm()
    {
        ...
        if ($currentEntity && !empty($currentEntity)) {
            if ($this->request->is(['post', 'put'])) {
                ...
                return $this->saveBulkEnrolment($currentEntity, new ArrayObject($currentData));
            }
        ...
        $process = function ($model, $entity) use ($event, $data) {
            ...
            if (empty($errors)) {
                if (array_key_exists($this->getAlias(), $dataArray)) {
                    ...
                    if (array_key_exists('students', $dataArray[$this->getAlias()])) {
                        foreach ($dataArray[$this->getAlias()]['students'] as $key => $value) {
                            if ($value['selected'] != 0) {
    ...
    public function saveBulkEnrolment(Entity $entity, ArrayObject $data)
    {
        $queryString = $this->getQueryString();
        $encodedQueryString = $this->paramsEncode($queryString);
        $primaryKey = $this->StudentEnrolment->getPrimaryKey();
        $url = [
            'plugin' => 'Institution',
            'controller' => 'Institutions',
        ...
        foreach ($data[$this->getAlias()]['students'] as $key => $studentObj) {
            if ($studentObj['selected']) {
                ...
                foreach ($entity->student_enrolment as $key => $value) {
                ...
                if ($this->StudentEnrolment->save($existingEntityToUpdate)) {
                    if (!empty($workflowAction->event_key)) {
                        $id = $existingEntityToUpdate->$primaryKey;
                        $subject = $this->StudentEnrolment;
        ...
        if ($WorkflowTransitions->saveMany($workflowTransitionEntities)) {
            ...
            $url['action'] = 'BulkStudentEnrolment';
            $url[0] = 'edit';
        }
        return $this->controller->redirect($url);
    }
    ...
    public function setupFields(Entity $entity = null)
    {
        ...
        $this->field('bulk_student_enrolment', ['entity' => $entity]);
    }
}
