    ...
    public function indexAfterAction(Event $event, Query $query, ResultSet $data, ArrayObject $extra)
    {
        ...
        if (!is_null($reportCardId) && !is_null($institutionId)) {
            ...
            if ($existingReportCard) {
                ...
                $generateAllUrl = $this->url('generateAll');
                $generateAllUrl['1'] = $encodedParams;
                unset($downloadAllPdfUrl['?']);
                unset($downloadAllUrl['?']);
                unset($generateAllUrl['?']);
                // POCOR-9165 end
                ...
                // Generate all button
                $generateButton['url'] = $generateAllUrl;
...
//        dd($params);
        ...
        $institutionId = $this->getInstitutionID();
        if ($hasTemplate) {
            $this->addReportCardsToProcesses($params['institution_id'],
...
//        dd($params);
    ...
    private function addReportCardsToProcesses($institutionId, $academicPeriodId, $reportCardId, $studentId = null)
    {
