...
class InstitutionRoomsTable extends ControllerActionTable
{
    ...
    public function onUpdateFieldSubjects(Event $event, array $attr, $action, ServerRequest $request)
    {
        ...
        if ($visibility && ($action == 'add' || $action == 'edit')) {

            $institutionId = $this->getInstitutionID();

            $attr['options'] = $this->getSubjectOptions(['institution_id' => $institutionId]);
        }
    ...
    private function checkIfCanEditOrDelete($entity)
    {
        ...
        if ($entity->room_status_id == $endOfUsageId) {    // If already end of usage, not allow to edit or delete
            $isEditable = false;
            $isDeletable = false;
        }
