...
class LandsTable extends ControllerActionTable
{
    ...
    public function initialize(array $config): void
    {
        ...
        // POCOR-9458 - Removed academic_period_id from scope due to field removal from table
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        return $validator
            ...
            ->add('start_date', [
                // POCOR-9458 - Commented out due to academic_period_id field removal from table
    ...
    public function indexBeforeQuery(EventInterface $event, Query $query, ArrayObject $extra)
    {
        ...
        // POCOR-9458 - Academic Period functionality commented out due to academic_period_id field removal from table
    ...
    public function viewEditBeforeQuery(EventInterface $event, Query $query, ArrayObject $extra)
    {
        ...
        $query->contain(['LandTypes', 'InfrastructureConditions']);
    }
    ...
    public function deleteOnInitialize(EventInterface $event, Entity $entity, Query $query, ArrayObject $extra)
    {
        ...
        if (!$results->isEmpty()) {
            foreach ($results as $obj) {
                // POCOR-9458 - Modified due to academic_period_id field removal from table
    ...
    // POCOR-9458 - onUpdateFieldAcademicPeriodId method commented out due to academic_period_id field removal from table
    ...
    public function onUpdateFieldEndDate(EventInterface $event, array $attr, $action, Request $request)
    {
        if ($action == 'view') {
            $attr['visible'] = false;
        } elseif ($action == 'add') {
    ...
    public function onUpdateFieldYearAcquired(EventInterface $event, array $attr, $action, Request $request)
    {
        if ($action == 'add') {
    ...
    {
        $this->setFieldOrder([
            // POCOR-9458 - Removed academic_period_id from field order due to field removal from table
            'change_type', 'institution_id', 'code', 'name', 'land_type_id', 'land_status_id', 'area', 'infrastructure_ownership_id', 'year_acquired', 'year_disposed', 'start_date', 'start_year', 'end_date', 'end_year', 'infrastructure_condition_id', 'previous_institution_land_id', 'new_land_type', 'new_start_date'
        ]);
    ...
    // POCOR-9458 - academicPeriodAfterSave method commented out due to academic_period_id field removal from table
    ...
    public function onExcelUpdateFields(EventInterface $event, ArrayObject $settings, $fields)
    {
        ...
        $InfrastructureCustomFields = TableRegistry::getTableLocator()->get('Infrastructure.LandCustomFields');
        if ($landType->name == 'Land') {
                    ...
                    foreach($customFieldData as $data) {
                        ...
                        if(!empty($data->number_value) && $data->field_type == 'CHECKBOX') {
                            ...
                            $fieldValue = $infrastructureCustomFieldOptions->find()
                                    ...
                                    ->innerJoin([$infrastructureCustomFields->getAlias() => $infrastructureCustomFields->getTable()],[
                        ...
                        if (!empty($data->number_value) && $data->field_type != 'CHECKBOX') {
                            $optvalue = TableRegistry::getTableLocator()->get('Infrastructure.InfrastructureCustomFieldOptions');
                            if($optvalue->exists(['id'=>$data->number_value])){
                                ...
                                $row[(string)$data->custom_field_id] = $opt;
                            }
                        }
                        if(!empty($data->number_value) && $data->field_type == 'NUMBER') {
                            $row[(string)$data->custom_field_id] = $data->number_value;
                        }
                        if(!empty($data->decimal_value)) {
                            $row[(string)$data->custom_field_id] = $data->decimal_value;
                        }
                        ...
                        if(!empty($data->time_value)) {
                            $row[(string)$data->custom_field_id] = $data->time_value;
                        }
                    }
                }
                return $row;
