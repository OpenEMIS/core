...
// Try save
                    ...
                    $handleCandidateNeededInfo = [
                        ...
                        'education_grade_id'   => $requestData['education_grade_id'] ?? null,
                        'academic_period_id'   => $requestData['academic_period_id'] ?? null,
                        // Optional: if you might use it in numbering logic later
                        'institution_class_id' => $requestData['institution_class_id'] ?? null,
                        'user_record_id' => $userRecordId
                    ];

// Generate candidate number only if we have the basics
                    if (!empty($handleCandidateNeededInfo['institution_id'])
                        && !empty($handleCandidateNeededInfo['education_grade_id'])
                        && !empty($handleCandidateNeededInfo['academic_period_id'])
                    ) {
...
//        Log::debug(print_r([__LINE__, __METHOD__, 'start'], true));
        ...
        // Inputs
        $institutionId     = (int)($requestData['institution_id'] ?? 0);
