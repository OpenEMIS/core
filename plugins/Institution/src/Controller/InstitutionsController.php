...
//                    Log::debug(print_r($fieldData, true));
        ...
        if (!empty($institutionId) && $studentAdmissionStatusValue == 0 && strtolower($studentAdmissionStatus) == "enrolled") {
            ...
            if($workflowStepData->workflow->name == 'Student Admission'){
                if (!empty($educationGradeId) && !empty($institutionId) && !empty($academicPeriodId)) {
                    ...
                    $this->handleCandidateNumber($requestData, $userRecordId); // POCOR-9323
    ...
    private function handleCandidateNumber(array $requestData, $userRecordId)
    {
        ...
        $autoGeneratedCandidateNumber = $ConfigItemTable
            ...
            ->first();
        if(empty($autoGeneratedCandidateNumber)
        ){
            return;
        }
        ...
        $institutionId = $requestData['institution_id'] ?? -1;
        $institutionClassId = $requestData['institution_class_id'] ?? -1;
        $educationGradeId = $requestData['education_grade_id'] ?? -1;
        $academicPeriodId = $requestData['academic_period_id'] ?? -1;
        // POCOR-9323 quick cleanup end
        ...
        $EducationGradesResult = $EducationGrades->find()
                    ...
                    ->first();
        if(empty($institutionResult) // POCOR-9323 quick cleanup
            || empty($AcademicPeriodsResult)
            || empty($EducationGradesResult)
        ){
            return;
        }
        ...
        $InstitutionStudentProgrammesData = $InstitutionStudentProgrammes->find()
            ...
            ->limit(1)
            ->first();
        if (empty($InstitutionStudentProgrammesData) // POCOR-9323 quick cleanup
        ) {
            return;
        }
        ...
        $data = $InstitutionStudentProgrammes->newEntity([
            ...
            'modified' => date('Y-m-d H:i:s'), // POCOR-9323 quick cleanup
            'created_user_id' => $this->Auth->user('id'),
            'created' => date('Y-m-d H:i:s') // POCOR-9323 quick cleanup
        ]);
...
//                Log::debug(print_r($entityTransferData, true));
    ...
    private function handleStaffInstitutionData($requestData, $userRecordId, $userId)
    {
        ...
        $institutionPositionId = $requestData['institution_position_id'] ?? null;
            ...
            $institutionStaffs = self::getDynamicTableInstance('institution_staff');
    ...
    private function handleShifts($requestData, $userRecordId)
    {
        ...
        if (!empty($shiftIds)) {
            ...
            self::triggerWebhooksForStaff($userRecordId); //POCOR-7971
        }
    }
...
// Check for an existing entity based on unique fields
            ...
            if ($existingEntity) {
                ...
                try {
                    return $StudentGuardians->save($existingEntity);
                } catch (\Exception $e) {
                    // Handle save error
                    throw $e;
                }
                ...
                try {
                    return $StudentGuardians->save($newEntity);
                } catch (\Exception $e) {
                    // Handle save error
                    throw $e;
                }
            ...
            try {
                return $StudentGuardians->save($entityGuardiansData, ['checkExisting' => true]);
            } catch (\Exception $e) {
                Log::debug(__FUNCTION__);
...
//                    $this->log($securityGroupUserViewArchiveAccessCount, 'debug');
    ...
    public function triggerWebhooksForStaff($userRecordId)
    {
        ...
        $shift =  self::getDynamicTableInstance('Institution.InstitutionShifts');
        $shiftData = $shift->find('all',
                            [ 'contain' => [
                                'ShiftOptions'
                            ],
