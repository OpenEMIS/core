...
<div class="overview-box alert" ng-class="disableElement">
	...
	<div class="data-section section_custom_1">
		...
		<div class="data-field">
			...
			<?= number_format($personalProfileCompletness['percentage']) .'%' ?>
			</h1>
		</div>
	</div>
	<div class="data-section section_custom_2">
		<div class="progress" style= "border-radius: 25px;height: 22px;">
			<div class="navbar progress-bar" role="progressbar"  style="background-color: #6699CC; width:<?= number_format($personalProfileCompletness['percentage']).'%' ?>">
			<?= number_format($personalProfileCompletness['percentage']).'%' ?>
			</div>
		</div>
	</div>
...
<div class="table-wrapper" ng-class="disableElement" id="profile-data-div" style="display:none">
   <div class="table-responsive">
      ...
      <tbody>
		...
		<?php
			unset($personalProfileCompletness['percentage']);
			foreach ($personalProfileCompletness as $pcVal) :
		?>
         <tr>
