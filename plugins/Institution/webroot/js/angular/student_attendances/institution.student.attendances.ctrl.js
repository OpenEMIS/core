angular
    .module("institution.student.attendances.ctrl", [
        "utils.svc",
        "alert.svc",
        "aggrid.locale.svc",
    ...
    .controller(
        "InstitutionStudentAttendancesCtrl",
        InstitutionStudentAttendancesController
    );

InstitutionStudentAttendancesController.$inject = [
    "$scope",
    "$q",
    "$window",
    "$http",
...
function InstitutionStudentAttendancesController(
    $scope,
    $q,
    $window,
    $http,
        ...
        if (vm.institutionId != null) {
            InstitutionStudentAttendancesSvc.getTranslatedText()
                ...
                .then(function (studentAbsenceReasonOptions) {
                    vm.studentAbsenceReasonOptions =
                        studentAbsenceReasonOptions;
                    vm.gridOptions.context.studentAbsenceReasons =
                        vm.studentAbsenceReasonOptions;
                    return InstitutionStudentAttendancesSvc.getAcademicPeriodOptions(
                        vm.institutionId
                    );
                }, vm.error)
                .then(function (academicPeriodOptions) {
                    vm.updateAcademicPeriodList(academicPeriodOptions);
                    return InstitutionStudentAttendancesSvc.getWeekListOptions(
                        vm.selectedAcademicPeriod
                    );
                }, vm.error)
                .then(function (weekListOptions) {
                    vm.updateWeekList(weekListOptions);
                    return InstitutionStudentAttendancesSvc.getDayListOptions(
                        vm.selectedAcademicPeriod,
                        vm.selectedWeek,
                        vm.institutionId
                    );
                ...
                .then(function (dayListOptions) {
                    ...
                    return InstitutionStudentAttendancesSvc.getClassOptions(
                        vm.institutionId,
                        vm.selectedAcademicPeriod
                    );
                }, vm.error)
                .then(function (classListOptions) {
                    ...
                    return InstitutionStudentAttendancesSvc.getEducationGradeOptions(
                        vm.institutionId,
                        vm.selectedAcademicPeriod,
                        vm.selectedClass
                    );
                ...
                //POCOR-8874 end
                // .then(function () {
                //     InstitutionStudentAttendancesSvc.isMarkableAttendance(
                //         vm.institutionId,
                //         vm.selectedAcademicPeriod,
                //         vm.selectedClass,
                //         vm.selectedDay
                ...
                //             vm.isMarkableAttendance = result; // Assign the resolved value
                //             console.log(
                //                 "Final isMarkableAttendance:",
                //                 vm.isMarkableAttendance
                //             );
                ...
                //             );
                //         });
                //     // vm.isMarkableAttendance = InstitutionStudentAttendancesSvc.isMarkableAttendance(vm.institutionId,vm.selectedAcademicPeriod,vm.selectedClass,vm.selectedDay);
                // }, vm.error)
    ...
    vm.updateAcademicPeriodList = function (academicPeriodOptions) {
        ...
        if (academicPeriodOptions.length > 0) {
            ...
            vm.selectedAcademicPeriod =
                academicPeriodOptions[selectedAcademicPeriodId].id;
        }
    };
    ...
    vm.updateWeekList = function (weekListOptions) {
        ...
        if (weekListOptions.length > 0) {
            for (var i = 0; i < weekListOptions.length; ++i) {
                if (
                    angular.isDefined(weekListOptions[i]["selected"]) &&
                    weekListOptions[i]["selected"]
                ) {
    ...
    vm.updateDayList = function (dayListOptions) {
        ...
        if (dayListOptions.length > 0) {
            for (var i = 0; i < dayListOptions.length; ++i) {
                if (
                    angular.isDefined(dayListOptions[i]["selected"]) &&
                    dayListOptions[i]["selected"]
                ) {
                    hasSelected = true;
                    ...
                    vm.schoolClosed =
                        angular.isDefined(dayListOptions[i]["closed"]) &&
                        dayListOptions[i]["closed"]
                            ? true
                            : false;
            ...
            if (!hasSelected) {
                ...
                vm.schoolClosed =
                    angular.isDefined(dayListOptions[0]["closed"]) &&
                    dayListOptions[0]["closed"]
                        ? true
                        : false;
                vm.gridOptions.context.date = vm.selectedDay;
                vm.gridOptions.context.schoolClosed = vm.schoolClosed;
    ...
    vm.updateClassList = function (classListOptions) {
        ...
        if (classListOptions.length > 0) {
            ...
            if (classListOptions[0].SecurityRoleFunctions) {
                vm.superAdmin = 0;
                vm.permissionView =
                    classListOptions[0].SecurityRoleFunctions._view;
                vm.permissionEdit =
                    classListOptions[0].SecurityRoleFunctions._edit;
            }
        }
    };
        ...
        if (attendancePeriodOptions.length > 0) {
            ...
            for (var i = 0; i < vm.dayListOptions.length; i++) {
                if (vm.dayListOptions[i].date === vm.selectedDay) {
                    //check date
                    if (
                        vm.dayListOptions[i].closed == true &&
                        vm.dayListOptions[i].periods.length != 0
                    ) {
                        for (
                            var j = 0;
                            j < vm.dayListOptions[i].periods.length;
                            j++
                        ) {
                            //   console.log(vm.selectedAttendancePeriod);
                            if (
                                vm.dayListOptions[i].periods[j] ==
                                vm.selectedAttendancePeriod
                            ) {
                                //check period
                                closedPeriod = true;
    ...
    vm.setColumnDef = function (noScheduledClicked) {
        ...
        if (vm.selectedDay != -1) {
            columnDefs =
                InstitutionStudentAttendancesSvc.getSingleDayColumnDefs(
                    vm.selectedAttendancePeriod,
                    noScheduledClicked,
                    vm.selectedSubject
                );
        } else {
            columnDefs = InstitutionStudentAttendancesSvc.getAllDayColumnDefs(
                vm.dayListOptions,
                vm.attendancePeriodOptions
            );
        }
    ...
    vm.countStudentData = function () {
        ...
        if (vm.selectedDay != -1) {
            ...
            if (vm.totalStudents > 0) {
                ...
                angular.forEach(
                    vm.classStudentList,
                    function (obj, studentKey) {
                        if (
                            angular.isDefined(obj.week_attendance) &&
                            Object.keys(obj.week_attendance).length > 0
                        ) {
    ...
    vm.getClassStudentParams = function () {
        if (vm.selectedAttendanceBy == "period") {
            vm.selectedSubject = 0;
        }
        vm.excelExportAUrl =
            vm.exportexcel +
    ...
    vm.changeAcademicPeriod = function () {
        ...
        UtilsSvc.isAppendLoader(true);
        InstitutionStudentAttendancesSvc.getWeekListOptions(
            vm.selectedAcademicPeriod
        )
            .then(function (weekListOptions) {
                vm.updateWeekList(weekListOptions);
    ...
    vm.changeWeek = function () {
        UtilsSvc.isAppendLoader(true);
        var weekObj = vm.weekListOptions.find(
            (obj) => obj.id == vm.selectedWeek
        );
        vm.selectedWeekStartDate = weekObj.start_day;
        vm.selectedWeekEndDate = weekObj.end_day;
        vm.gridOptions.context.week = vm.selectedWeek;
        InstitutionStudentAttendancesSvc.getDayListOptions(
            vm.selectedAcademicPeriod,
            vm.selectedWeek,
            vm.institutionId
        )
    ...
    vm.changeDay = function () {
        UtilsSvc.isAppendLoader(true);
        var dayObj = vm.dayListOptions.find(
            (obj) => obj.date == vm.selectedDay
        );
        vm.schoolClosed =
            angular.isDefined(dayObj.closed) && dayObj.closed ? true : false;
        vm.gridOptions.context.schoolClosed = vm.schoolClosed;
        vm.gridOptions.context.date = vm.selectedDay;
        ...
        InstitutionStudentAttendancesSvc.getSubjectOptions(
            vm.institutionId,
            vm.selectedClass,
            vm.selectedAcademicPeriod,
            vm.selectedDay,
    ...
    vm.changeAttendanceBy = function () {
        ...
        InstitutionStudentAttendancesSvc.getEducationGradeOptions(
            ...
            .finally(function () {
                if (vm.selectedAttendanceBy == "subject") {
                    vm.selectedAttendancePeriod = "";
                } else {
                    vm.selectedSubject = "";
                }
