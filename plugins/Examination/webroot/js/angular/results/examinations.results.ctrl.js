angular
    .module("examinations.results.ctrl", [
        "utils.svc",
        "alert.svc",
        "aggrid.locale.svc",
...
ExaminationsResultsController.$inject = [
    "$scope",
    "$anchorScroll",
    "$filter",
    "$q",
    ...
    "AlertSvc",
    "AggridLocaleSvc",
    "ExaminationsResultsSvc",
];

function ExaminationsResultsController(
    $scope,
    $anchorScroll,
    $filter,
    $q,
    ...
    angular.element(document).ready(function () {
        ...
        if (!vm.examinationId || !vm.examinationCentreId) {
            AlertSvc.error(
                vm,
                "Missing required examination parameters. Please check the URL."
            );
            return;
        }
        ...
        console.log("Initializing with:", {
            ...
            examinationCentreId: vm.examinationCentreId,
        });

        ExaminationsResultsSvc.init(angular.baseUrl);
        UtilsSvc.isAppendLoader(true);
        ExaminationsResultsSvc.getExaminationCentreExaminations(
            vm.examinationCentreId,
            vm.examinationId
        )
            ...
            .then(
                function (response) {
                    ...
                    vm.examinationOptions = [
                        {
                            text: examinationData.code_name,
                            value: examinationData.id,
                        },
                    ];
                    vm.examinationCentreOptions = [
                        {
                            text: examinationCentreData.code_name,
                            value: examinationCentreData.id,
                        },
                    ];
                    return ExaminationsResultsSvc.getSubjects(
                        vm.examinationId,
                        vm.examinationCentreId
                    );
                },
                ...
                vm.gridOptions = {
                    ...
                    onCellValueChanged: function (params) {
                        ...
                        if (
                            ...
                            vm.results[params.data.student_id][institutionId] =
                                { marks: "" };
                        }
                        ...
                        params.data.total_mark =
                            ExaminationsResultsSvc.calculateTotal(params.data);
    ...
    function onChangeSubject(examinationId, subject) {
        ...
        if (vm.gridOptions != null) {
            ...
            vm.gridOptions.context.examination_centre_id =
                vm.examinationCentreId;
            vm.gridOptions.context.education_subject_id =
                subject.education_subject_id;
        ...
        var dataSource = {
            ...
            getRows: function (params) {
                ...
                ExaminationsResultsSvc.getRowData(
                    examinationId,
                    vm.examinationCentreId,
                    subject,
                    limit,
        ...
        ExaminationsResultsSvc.getColumnDefs(action, subject, vm.results)
            .then(
                function (res) {
                    ...
                    ExaminationsResultsSvc.translate(textToTranslate).then(
                        function (respond) {
    ...
    function onSyncClick() {
        ...
        if (!vm.examinationId || !vm.examinationCentreId) {
            AlertSvc.error(
                vm,
                "Missing examination or examination centre information. Please refresh the page."
            );
            return;
        }
        ...
        console.log("examinationCentreId:", examinationCentreId);
        console.log("educationSubjectId:", educationSubjectId);
        console.log("examinationItemId:", examinationItemId);

        // Show loading message
        AlertSvc.info(vm, "Synchronization in progress...");

        // Add spinner to indicate loading
        UtilsSvc.isAppendSpinner(true, "examination-result-table");

        ExaminationsResultsSvc.syncResults(
            vm.examinationId,
            vm.examinationCentreId,
            educationSubjectId,
            examinationItemId
            ...
            .then(
                function (response) {
                    ...
                    // Check if response contains success/failure information
                ...
                function (error) {
                    ...
                    if (error.data && error.data.message) {
                        ...
                        errorMessage = "Sync failed: " + error.statusText;
                    } else if (error.status) {
                        errorMessage =
                            "Sync failed with status: " + error.status;
                    }

                    AlertSvc.error(vm, errorMessage);
                }
            )
            .finally(function () {
    ...
    function onSaveClick() {
        if (vm.gridOptions != null) {
            var examinationId = vm.gridOptions.context.examination_id;
            var examinationCentreId =
                vm.gridOptions.context.examination_centre_id;
            var educationSubjectId =
                vm.gridOptions.context.education_subject_id;
            var examinationItemId =
                vm.gridOptions.context.examination_subject_id;

            UtilsSvc.isAppendSpinner(true, "examination-result-table");
            ExaminationsResultsSvc.saveRowData(
                vm.results,
                vm.education_subject,
                examinationId,
                examinationCentreId,
