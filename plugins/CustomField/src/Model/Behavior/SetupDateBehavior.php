...
class SetupDateBehavior extends SetupBehavior {
    ...
    public function onSetDateElements(Event $event, Entity $entity) {
		...
		$this->_table->ControllerAction->field('validation_rules_date', ['options' => $this->rangeValidationOptions, 'onChangeReload' => true, 'after' => 'is_mandatory', 'default' => $selectedRangeValidation, 'attr' => ['required' => 'required', 'label' => $this->_table->getMessage('general.validationRules')]]);
		...
		if (!empty($selectedRangeValidation)) {
			switch ($selectedRangeValidation) {
				case 'earlier':
					...
					$this->_table->ControllerAction->field('start_date', $options);
					break;
				case 'later':
					$options = ['type' => 'date', 'after' => 'validation_rules_date', 'null' => false];
        			...
        			$this->_table->ControllerAction->field('end_date', $options);
					break;
				case 'between':
					$options = ['type' => 'date', 'after' => 'validation_rules_date', 'null' => false];
					if (array_key_exists('start_date', $paramsArray)) {
