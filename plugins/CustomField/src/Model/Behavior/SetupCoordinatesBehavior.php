...
class SetupCoordinatesBehavior extends SetupBehavior {
	...
	public function editAfterQuery(Event $event, Entity $entity) {
		...
		if (isset($fieldType) && $fieldType == 'COORDINATES') {
			// $this->addCoordinatesValidation();
		}
	}
	...
	private function addCoordinatesValidation() {
    ...
    public function onSetCoordinatesElements(Event $event, Entity $entity) {
  ...
  //       			$this->_table->ControllerAction->field('end_time', $options);
	...
	// public function onGetValidationRulesCoordinates(Event $event, Entity $entity) {
	// 	$paramsArray = (!empty($entity->params))? json_decode($entity->params, true): [];
	// 	if (array_key_exists('start_time', $paramsArray) && array_key_exists('end_time', $paramsArray)) {
