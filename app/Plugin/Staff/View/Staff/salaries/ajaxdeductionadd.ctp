...
<tr>
	<td>
		<?php echo $this->Form->input('StaffSalaryDeduction.' . $index . '.id', array('type' => 'hidden', 'class' => 'deduction-control-id', 'label' => false, 'value' => "")); ?>
		<?php echo $this->Form->input('StaffSalaryDeduction.' . $index . '.salary_deduction_type_id', array('class' => 'form-control', 'label' => false, 'options' => $categories)); ?>
	</td>
	<td>
		<?php
		echo $this->Form->input('StaffSalaryDeduction.' . $index . '.deduction_amount', array(
	...
	<td> 
		<span class="icon_delete" title="<?php echo $this->Label->get('general.delete'); ?>" onClick="Salary.deleteDeduction(this)"></span>
	</td>
</tr>