...
<div id="student" class="content_wrapper edit add">
	<h1>
		...
		if(!empty($this->data[$modelName]['id'])){
			echo $this->Html->link(__('View'), array('action' => 'health_consultation_view', $this->data[$modelName]['id']), array('class' => 'divider'));
		}
		else{
			echo $this->Html->link(__('List'), array('action' => 'health_consultation'), array('class' => 'divider'));
		}
		?>
	</h1>
	...
	echo $this->Form->create($modelName, array(
		'url' => array('controller' => 'Staff', 'action' => 'health_consultation_add', 'plugin'=>'Staff'),
		'type' => 'file',
    ...
    <div class="row">
        ...
        <div class="value">
		<?php 
			echo $this->Form->input('health_consultation_type_id', array(
									'options' => $healthConsultationsOptions,
									'label' => false)
									); 
		?>
        </div>
    ...
    <div class="row">
        ...
        <div class="value"><?php echo $this->Form->input('treatment');?></div>
    </div>
	
	<div class="controls view_controls">
		<input type="submit" value="<?php echo __("Save"); ?>" class="btn_save btn_right" onclick="return Config.checkValidate();"/>
		<?php echo $this->Html->link(__('Cancel'), array('action' => 'health_consultation'), array('class' => 'btn_cancel btn_left')); ?>
	</div>
