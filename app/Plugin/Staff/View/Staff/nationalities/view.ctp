...
<div id="nationalityView" class="content_wrapper">
    <h1>
        ...
        <?php
        $data = $nationalityObj[0]['StaffNationality'];
        echo $this->Html->link(__('List'), array('action' => 'nationalities', $data['staff_id']), array('class' => 'divider'));
        if($_edit) {
...
echo $this->Html->link($this->Label->get('general.list'), array('action' => 'nationalities'), array('class' => 'divider'));
if ($_edit) {
    echo $this->Html->link($this->Label->get('general.edit'), array('action' => 'nationalitiesEdit', $id), array('class' => 'divider'));
}
if ($_delete) {
    echo $this->Html->link($this->Label->get('general.delete'), array('action' => 'nationalitiesDelete'), array('class' => 'divider', 'onclick' => 'return jsForm.confirmDelete(this)'));
}
$this->end();
