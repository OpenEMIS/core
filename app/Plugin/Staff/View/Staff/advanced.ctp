...
'options' => $identityTypeOptions, 'empty'=>'', 'after'=>'</div>'.$this->Form->input('identity', array('type' => 'text', 'div'=>false, 'label'=>false, 'value' => $session->read('Staff.AdvancedSearch.Search.identity'))), 'value' => $session->read('Staff.AdvancedSearch.Search.identity_type_id')));
