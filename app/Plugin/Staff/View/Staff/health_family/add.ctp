...
<div id="student" class="content_wrapper edit add">
	<h1>
		...
		if(!empty($this->data[$modelName]['id'])){
			echo $this->Html->link(__('View'), array('action' => 'health_family_view', $this->data[$modelName]['id']), array('class' => 'divider'));
		}
		else{
			echo $this->Html->link(__('List'), array('action' => 'health_family'), array('class' => 'divider'));
		}
		?>
	</h1>
	...
	echo $this->Form->create($modelName, array(
		'url' => array('controller' => 'Staff', 'action' => 'health_family_add', 'plugin'=>'Staff'),
		'type' => 'file',
    ...
    <div class="row">
        ...
        <div class="value">
		<?php 
			echo $this->Form->input('health_relationship_id', array(
									'options' => $healthRelationshipsOptions,
									'label' => false)
									); 
		?>
        </div>
	...
	<div class="controls view_controls">
		...
		<?php echo $this->Html->link(__('Cancel'), array('action' => 'health_family'), array('class' => 'btn_cancel btn_left')); ?>
	</div>
