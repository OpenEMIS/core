...
class StudentHealthConsultation extends StudentsAppModel {
	...
	public function healthConsultationView($controller, $params) {
		...
		$controller->Session->write('StudentHealthConsultation.id', $id);
		$fields = $this->getDisplayFields($controller);
		$controller->set(compact('header', 'data', 'fields', 'id'));
	}

	public function healthConsultationDelete($controller, $params) {
		if ($controller->Session->check('StudentHealthConsultation.id')) {
			$id = $controller->Session->read('StudentHealthConsultation.id');
			if ($this->delete($id)) {
				$controller->Message->alert('general.delete.success');
			} else {
				$controller->Message->alert('general.delete.failed');
			}
			$controller->Session->delete('StudentHealthConsultation.id');
			return $controller->redirect(array('action' => 'healthConsultation'));
		}
	}
