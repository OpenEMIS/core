...
class StudentHealthHistory extends StudentsAppModel {
	...
	public function healthHistoryView($controller, $params) {
		...
		$controller->Session->write('StudentHealthHistory.id', $id);
		$fields = $this->getDisplayFields($controller);
		$controller->set(compact('header', 'data', 'fields', 'id'));
	}

	public function healthHistoryDelete($controller, $params) {
		if ($controller->Session->check('StudentHealthHistory.id')) {
			$id = $controller->Session->read('StudentHealthHistory.id');
			if ($this->delete($id)) {
				$controller->Message->alert('general.delete.success');
			} else {
				$controller->Message->alert('general.delete.failed');
			}
			$controller->Session->delete('StudentHealthHistory.id');
			return $controller->redirect(array('action' => 'healthHistory'));
		}
	}
