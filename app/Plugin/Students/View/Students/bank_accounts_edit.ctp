		...
		<div class="table">
			...
			if(count($data) > 0){
				...
				foreach($data as $arrVal){
				   $bnkbrnchdata = array();
				   echo '<div class="table_row" id="bankaccount_row_'.$arrVal['StudentBankAccount']['id'].'">
							<div class="table_cell">
								<input type="hidden" name="data[StudentBankAccount]['.$ctr.'][id]" value="'.$arrVal['StudentBankAccount']['id'].'" />
								<input type="hidden" name="data[StudentBankAccount]['.$ctr.'][active]" value="0" />
								<input name="data[StudentBankAccount][active]" value="'.$arrVal['StudentBankAccount']['id'].'" type="radio" '.($arrVal['StudentBankAccount']['active'] == 1?'checked':'').'/>
							</div>
							<div class="table_cell">
								<div class="input_wrapper">
									<input type="text" name="data[StudentBankAccount]['.$ctr.'][account_name]" value="'.$arrVal['StudentBankAccount']['account_name'].'" />
								</div>
							</div>
							<div class="table_cell">
								<div class="input_wrapper">
									<input type="text" name="data[StudentBankAccount]['.$ctr.'][account_number]" value="'.$arrVal['StudentBankAccount']['account_number'].'"/>
								</div>
							</div>
							<div class="table_cell">
							...
							foreach ($bank as $arrBankandBranches) {
								...
								foreach($arrBankandBranches['BankBranch'] as $arrIn){
									if($arrIn['id'] == $arrVal['StudentBankAccount']['bank_branch_id']){
							...
							<div class="table_cell">
								<select name="data[StudentBankAccount]['.$ctr.'][bank_branch_id]">
									<option value="0">' . __('--Select--') . '</option>';
								foreach($bnkbrnchdata as $arrbranches){
									echo '<option value="'.$arrbranches['id'].'" '.($arrbranches['id'] == $arrVal['StudentBankAccount']['bank_branch_id']?' selected="selected" ':"").'>'.$arrbranches['name'].'</option>';
								}
							echo '</select>
							</div>
							<div class="table_cell"><span class="icon_delete" title="'.__("Delete").'" onClick="BankAccounts.confirmDeletedlg('.$arrVal['StudentBankAccount']['id'].')"></span></div>
					</div>';
				   $ctr++;
				}
			}
