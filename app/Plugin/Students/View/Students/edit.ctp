	...
	<?php 
		...
		if ($autoid=='') {
			...
			if ($this->Session->check('Student.id')) {
				echo $this->Form->input('SecurityUser.openemis_no', array('label' => $openEmisIdLabel));
			} else {
				echo $this->Form->input('SecurityUser.openemis_no', array('label' => $openEmisIdLabel, 'value' => $autoid));
			}
		}
		...
		echo $this->Form->input('SecurityUser.gender', array('options' => $genderOptions));
		$tempDob = isset($this->data['SecurityUser']['date_of_birth']) ? array('data-date' => $this->data['SecurityUser']['date_of_birth']) : array();
		echo $this->FormUtility->datepicker('SecurityUser.date_of_birth', $tempDob);
