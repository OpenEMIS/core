...
<fieldset class="section_break">
	...
	<?php 
		...
		if ($configStudentContact != 'Excluded') {
			if (isset($contactOptions)) {
				$selectAndTxtOptions = array(
					'label' => __('Contact'),
					'selectOptions' => $contactOptions,
					'selectId' => 'StudentContact.0.contact_type_id',
					'txtId' => 'StudentContact.0.value',
		...
		if ($configStudentSpecialNeed != 'Excluded') {
			if (isset($specialNeedOptions)) {
				$selectAndTxtOptions = array(
					...
					'selectId' => 'StudentSpecialNeed.0.special_need_type_id',
					'txtId' => 'StudentSpecialNeed.0.comment',
					'txtPlaceHolder' => __('Comment')
				);
				echo $this->element('templates/selectAndTxt', $selectAndTxtOptions);
				echo $this->Form->hidden('StudentSpecialNeed.0.special_need_date', array('value' => date("Y-m-d")));
			}
		}
