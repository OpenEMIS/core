...
<div id="bankAccountView" class="content_wrapper">
    <h1>
		...
		<?php
		$data = $bankAccountObj[0]['StudentBankAccount'];
...
echo $this->Html->link($this->Label->get('general.list'), array('action' => 'bankAccounts'), array('class' => 'divider'));
if($_edit) {
	echo $this->Html->link($this->Label->get('general.edit'), array('action' => 'bankAccountsEdit', $data[$model]['id']), array('class' => 'divider'));
}
if($_delete) {
	echo $this->Html->link($this->Label->get('general.delete'), array('action' => 'bankAccountsDelete'), array('class' => 'divider', 'onclick' => 'return jsForm.confirmDelete(this)'));
}
$this->end();
