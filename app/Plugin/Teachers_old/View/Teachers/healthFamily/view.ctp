...
<div id="student" class="content_wrapper">
	<h1>
		...
		<?php
			...
			if($_edit) {
				echo $this->Html->link(__('Edit'), array('action' => 'healthFamilyEdit',$obj['id'] ), array('class' => 'divider'));
			}
			if($_delete) {
				echo $this->Html->link(__('Delete'), array('action' => 'healthFamilyDelete'), array('class' => 'divider', 'onclick' => 'return jsForm.confirmDelete(this)'));
			}
		?>
	</h1>
