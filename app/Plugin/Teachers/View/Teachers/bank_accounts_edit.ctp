		...
		<div class="table">
			...
			if(count($data) > 0){
				...
				foreach($data as $arrVal){
				   $bnkbrnchdata = array();
				   echo '<div class="table_row" id="bankaccount_row_'.$arrVal['TeacherBankAccount']['id'].'">
							<div class="table_cell">
								<input type="hidden" name="data[TeacherBankAccount]['.$ctr.'][id]" value="'.$arrVal['TeacherBankAccount']['id'].'" />
								<input type="hidden" name="data[TeacherBankAccount]['.$ctr.'][active]" value="0" />
								<input name="data[TeacherBankAccount][active]" value="'.$arrVal['TeacherBankAccount']['id'].'" type="radio" '.($arrVal['TeacherBankAccount']['active'] == 1?'checked':'').'/>
							</div>
							<div class="table_cell">
								<div class="input_wrapper">
									<input type="text" name="data[TeacherBankAccount]['.$ctr.'][account_name]" value="'.$arrVal['TeacherBankAccount']['account_name'].'" />
								</div>
							</div>
							<div class="table_cell">
								<div class="input_wrapper">
									<input type="text" name="data[TeacherBankAccount]['.$ctr.'][account_number]" value="'.$arrVal['TeacherBankAccount']['account_number'].'"/>
								</div>
							</div>
							<div class="table_cell">
							...
							foreach ($bank as $arrBankandBranches) {
								...
								foreach($arrBankandBranches['BankBranch'] as $arrIn){
									if($arrIn['id'] == $arrVal['TeacherBankAccount']['bank_branch_id']){
							...
							<div class="table_cell">
								<select name="data[TeacherBankAccount]['.$ctr.'][bank_branch_id]">
									<option value="0">' . __('--Select--') . '</option>';
								foreach($bnkbrnchdata as $arrbranches){
									echo '<option value="'.$arrbranches['id'].'" '.($arrbranches['id'] == $arrVal['TeacherBankAccount']['bank_branch_id']?' selected="selected" ':"").'>'.$arrbranches['name'].'</option>';
								}
							echo '</select>
							</div>
							<div class="table_cell"><span class="icon_delete" title="'.__("Delete").'" onClick="BankAccounts.confirmDeletedlg('.$arrVal['TeacherBankAccount']['id'].')"></span></div>
					</div>';
				   $ctr++;
				}
			}
