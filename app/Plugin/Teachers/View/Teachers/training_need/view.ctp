...
<div id="license" class="content_wrapper">
			...
			if($_execute) {
				if($obj['training_status_id'] == 2 || $obj['training_status_id']==3){
					if($obj['training_status_id'] == 2){
						echo $this->Html->link(__('Activate'), array('action' => 'trainingNeedActivate'), array('class' => 'divider', 'onclick' => 'return jsForm.confirmDelete(this)'));
					}
					echo $this->Html->link(__('Inactivate'), array('action' => 'trainingNeedInactivate'), array('class' => 'divider', 'onclick' => 'return jsForm.confirmDelete(this)'));
				}
			}
		?>
	</h1>
	<?php echo $this->element('alert'); ?>
		...
		<div class="row">
			<div class="label"><?php echo __('Course Descriptiom'); ?></div>
