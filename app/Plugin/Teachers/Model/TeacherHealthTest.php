...
class TeacherHealthTest extends TeachersAppModel {
	...
	public function health_test_view($controller, $params){
		...
		$controller->Session->write('TeacherHealthTestId', $id);
		$HealthTestType = ClassRegistry::init('HealthTestType');
		$healthTestsOptions = $HealthTestType->find('list', array('fields'=> array('id', 'name')));
	...
	public function health_test_delete($controller, $params) {
        if($controller->Session->check('TeacherId') && $controller->Session->check('TeacherHealthTestId')) {
            $id = $controller->Session->read('TeacherHealthTestId');
			...
			$controller->Session->delete('TeacherHealthTestId');
            $controller->redirect(array('action' => 'health_test'));
        }
    }
