...
class TeachersController extends TeachersAppController {
    ...
    public $modules = array(
        'healthHistory' => 'Teachers.TeacherHealthHistory',
        'healthFamily' => 'Teachers.TeacherHealthFamily',
        'healthImmunization' => 'Teachers.TeacherHealthImmunization',
        'healthMedication' => 'Teachers.TeacherHealthMedication',
        'healthAllergy' => 'Teachers.TeacherHealthAllergy',
        'healthTest' => 'Teachers.TeacherHealthTest',
        'healthConsultation' => 'Teachers.TeacherHealthConsultation',
    ...
    public function beforeFilter() {
        ...
        if (in_array($this->action, $actions)) {
            $this->bodyTitle = 'Teachers';
             if($this->action=='add'){
                $this->Session->delete('TeacherId');
                $this->Session->write('WizardMode', true);
                $wizardLink = $this->Navigation->getWizardLinks('Teacher');
            ...
            }else if (!$this->Session->check('TeacherId') && $this->action !== 'Home') {
                $name = __('New Teacher');
                $this->bodyTitle = $name;
            }
        }
    ...
    public function edit() {
        ...
        if($this->Session->check('TeacherId')){
            $this->Teacher->id = $this->Session->read('TeacherId');
    ...
    public function additionalEdit() {
        ...
        if ($this->request->is('post')) {
            ...
            foreach ($arrFields as $fieldVal) {
                ...
                if (!isset($this->request->data['TeacherCustomValue'][$fieldVal]))
                    continue;
                foreach ($this->request->data['TeacherCustomValue'][$fieldVal] as $key => $val) {
                    ...
                    if ($fieldVal == "checkbox") {
                        ...
                        $this->TeacherCustomValue->create();
                        if ($datafields)
                            $this->TeacherCustomValue->id = $datafields['TeacherCustomValue']['id'];
                        $arrV['teacher_custom_field_id'] = $key;
    ...
    public function contactsAdd() {
        ...
        if ($this->request->is('post')) {
            ...
            $contactData['teacher_id'] = $this->teacherId;
        ...
        if ($this->request->is('get')) {
           ...
           if(isset($this->data['submit']) && $this->data['submit']=='Skip'){
            ...
            if ($this->TeacherContact->save($contactData)) {
                ...
                $this->Navigation->updateWizard($this->action,$contactId);
                $this->Utility->alert($this->Utility->getMessage('SAVE_SUCCESS'));
