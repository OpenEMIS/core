	...
	if (!empty($tableRowData)) {
		...
		foreach ($tableRowData as $obj) {
			if (empty($selectedSourceIds)) {
				...
				$checked = (in_array($obj['IC_NId'], $selectedSourceIds)) ? 'checked' : false;
			}
			...
			$input = $this->Form->input('IndicatorClassification.IC_NId.' . $i, $bodyFirstColOptions);

			$row = array();
			$row[] = array($input, array('class' => $additionalClass));
			$row[] = array($obj['IC_Name'], array('class' => 'data-list'));
			$tableData[] = $row;
			$i++;
		}
	}
