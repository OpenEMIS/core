...
class BatchShell extends AppShell {
    public $uses = array('BatchProcess','Reports.Report','Reports.BatchReport', 'Datawarehouse.DatawarehouseIndicator');
    ...
    public function run() {
        ...
        $data = $this->BatchProcess->find('list',array('fields'=>array('id','reference_id','reference_table'),'conditions'=>array('status'=>1,'reference_table'=>array('reports', 'datawarehouse_indicators'))));
               
        //PROCESS each item in QUEUE
        foreach($data as $table => $val){
          $id = key($val);
          $ReportRef_id = $data[$table][$id];
          ...
          try{
               ...
               $ReportRec = array();
               $indicator = true;
      			   if($table!='datawarehouse_indicators'){
                  $ReportRec = $this->Report->findById($ReportRef_id);
      			   }else{
                  $ReportRec = $this->DatawarehouseIndicator->findById($ReportRef_id);
                  $indicator = false;
               }
      			   
      			   $this->genReport($ReportRec,$id, $indicator);
    ...
    public function genReport($reportRec,$BatchProcessId, $indicator=true){ 
