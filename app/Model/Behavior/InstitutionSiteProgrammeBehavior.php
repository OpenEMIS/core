...
class InstitutionSiteProgrammeBehavior extends ModelBehavior {
	public function getConditionsByAcademicPeriodId(Model $model, $academicPeriodId=0) {
		$conditions = array();
		if($academicPeriodId > 0) {
			...
			$conditions = array();
			$conditions[$model->alias.'.start_date <='] = $endDate;
			$conditions['OR'] = array(
				$model->alias.'.end_date >=' => $startDate,
				$model->alias.'.end_date IS NULL'
			);
		}
