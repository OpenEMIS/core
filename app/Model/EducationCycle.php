...
class EducationCycle extends AppModel {
	public $actsAs = array('ControllerAction2', 'Reorder' => array('parentKey' => 'education_level_id'));
	...
	public function getCycles($options = array()) {
		...
		$listOnly = (array_key_exists('listOnly', $options))? $options['listOnly']: false;
		$type = ($listOnly)? 'list': 'all';
		$records = $this->find($type, array('conditions' => array('EducationCycle.visible' => 1)));
		if ($type == 'all') {
