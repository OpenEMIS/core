...
<div id="setup-variables" class="content_wrapper edit finance_categories">
	...
	<?php foreach($category as $type) { ?>
		<?php foreach($type['items'] as $nature => $options) { ?>
		<fieldset class="section_group">
			<legend><?php echo __($nature) ?></legend>
			
			<?php foreach($options['options'] as $typeName => $categories) { ?>
			<fieldset class="section_break">
				<legend><?php echo __($typeName) ?></legend>
				<?php			
				echo $this->Form->hidden('model', array('id' => 'model', 'value' => $options['model']));
				
				if(isset($categories['conditions'])) {
					foreach($categories['conditions'] as $conditionName => $conditionValue) {
				...
				foreach($categories['options'] as $i => $values) {
