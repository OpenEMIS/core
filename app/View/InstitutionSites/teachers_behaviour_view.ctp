...
<div id="teacherBehaviourView" class="content_wrapper">
    <h1>
        <span><?php echo __('Behaviour Details'); ?></span>
		<?php
		$data = $teacherBehaviourObj[0]['TeacherBehaviour'];
		echo $this->Html->link(__('List'), array('action' => 'teachersBehaviour', $data['teacher_id']), array('class' => 'divider'));
		if($_edit) {
			echo $this->Html->link(__('Edit'), array('action' => 'teachersBehaviourEdit', $data['id']), array('class' => 'divider'));
		}
		if($_delete) {
			echo $this->Html->link(__('Delete'), array('action' => 'teachersBehaviourDelete'), array('class' => 'divider', 'onclick' => 'return jsForm.confirmDelete(this)'));
		}
		?>
    </h1>
