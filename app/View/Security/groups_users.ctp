...
<div id="roles" class="content_wrapper edit">
	<h1>
		...
		<?php
		if($_edit) {
			echo $this->Html->link(__('Edit'), array('action' => 'groupsEdit', $group['id']), array('class' => 'divider'));
		}
		?>
	</h1>
	...
	<fieldset class="section_group">
		...
		<fieldset class="section_break">
			<legend><?php echo sprintf('%s (%d)', $obj['name'], count($obj['users'])) ; ?></legend>
			...
			<div class="table_scrollable group_user_list <?php echo count($obj['users']) > 4 ? 'scroll_active' : ''; ?>">
				...
				<div class="list_wrapper" limit="4">
					<div class="table">
						<div class="table_body">
							<?php foreach($obj['users'] as $user) { ?>
							<div class="table_row">
								<div class="table_cell cell_id_no"><?php echo $user['identification_no']; ?></div>
								<div class="table_cell"><?php echo trim($user['first_name'] . ' ' . $user['last_name']); ?></div>
								<div class="table_cell cell_icon_action">
									<span class="icon_delete" url="<?php echo sprintf('Security/groupsUserRemove/%d/%d', $roleId, $user['id']); ?>" onClick="Security.removeGroupUser(this)"></span>
								</div>
							</div>
							<?php } ?>
						</div>
