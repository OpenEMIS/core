...
UPDATE `navigations` SET `action` = 'staff', `pattern` = 'staff' 
WHERE `module` LIKE 'Institution' AND `controller` LIKE 'Census' AND `title` LIKE 'Staff';

UPDATE `security_functions` SET `_view` = 'staff', `_edit` = '_view:staffEdit' 
