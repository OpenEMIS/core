...
SELECT main_query.academic_period_id
    ...
    ,main_query.institution_id
    ,main_query.institution_code
    ,main_query.institution_name
    ,main_query.education_grade_id
    ,main_query.education_grade_code
    ,main_query.education_grade_name
    ,main_query.institution_class_id
    ,main_query.institution_class_name
    ,dates_generator.date_info attendance_date
    ,attendance_marking_type.period_id
    ,attendance_marking_type.period_name
    ,attendance_marking_type.subject_id
    ,attendance_marking_type.subject_name
    ,main_query.total_female_count female_count
    ,main_query.total_male_count male_count
    ,main_query.total_male_count + main_query.total_female_count total_count
    ,CASE WHEN IF(marked_attendance.institution_id IS NOT NULL, IFNULL(marked_attendance.present_female_count, 0) + IFNULL(marked_attendance.present_male_count, 0), 0) < 0 THEN 0 ELSE IF(marked_attendance.institution_id IS NOT NULL, IFNULL(marked_attendance.present_female_count, 0) + IFNULL(marked_attendance.present_male_count, 0), 0) END marked_attendance
    ,IF(marked_attendance.institution_id IS NULL, main_query.total_male_count + main_query.total_female_count, 0) unmarked_attendance
    ,CASE WHEN IFNULL(marked_attendance.present_female_count, 0) - IFNULL(marked_absence.absent_female_count, 0) < 0 THEN 0 ELSE IFNULL(marked_attendance.present_female_count, 0) - IFNULL(marked_absence.absent_female_count, 0) END present_female_count
    ,CASE WHEN IFNULL(marked_attendance.present_male_count, 0) - IFNULL(marked_absence.absent_male_count, 0) < 0 THEN 0 ELSE IFNULL(marked_attendance.present_male_count, 0) - IFNULL(marked_absence.absent_male_count, 0) END present_male_count
    ,CASE WHEN IFNULL(marked_attendance.present_female_count, 0) + IFNULL(marked_attendance.present_male_count, 0) - (IFNULL(marked_absence.absent_female_count, 0) + IFNULL(marked_absence.absent_male_count, 0)) < 0 THEN 0 ELSE IFNULL(marked_attendance.present_female_count, 0) + IFNULL(marked_attendance.present_male_count, 0) - (IFNULL(marked_absence.absent_female_count, 0) + IFNULL(marked_absence.absent_male_count, 0)) END present_total_count
    ,CASE WHEN IFNULL(marked_absence.absent_female_count, 0) < 0 THEN 0 ELSE IFNULL(marked_absence.absent_female_count, 0) END absent_female_count
    ,CASE WHEN IFNULL(marked_absence.absent_male_count, 0) < 0 THEN 0 ELSE IFNULL(marked_absence.absent_male_count, 0) END absent_male_count
    ,CASE WHEN IFNULL(marked_absence.absent_female_count, 0) + IFNULL(marked_absence.absent_male_count, 0) < 0 THEN 0 ELSE IFNULL(marked_absence.absent_female_count, 0) + IFNULL(marked_absence.absent_male_count, 0) END absent_total_count
    ,CASE WHEN IFNULL(marked_absence.late_female_count, 0) < 0 THEN 0 ELSE IFNULL(marked_absence.late_female_count, 0) END late_female_count
    ,CASE WHEN IFNULL(marked_absence.late_male_count, 0) < 0 THEN 0 ELSE IFNULL(marked_absence.late_male_count, 0) END late_male_count
    ,CASE WHEN IFNULL(marked_absence.late_female_count, 0) + IFNULL(marked_absence.late_male_count, 0) < 0 THEN 0 ELSE IFNULL(marked_absence.late_female_count, 0) + IFNULL(marked_absence.late_male_count, 0) END late_total_count
    ,CURRENT_TIMESTAMP
FROM
(
    SELECT institution_class_students.academic_period_id
        ...
        ,COUNT(DISTINCT(CASE WHEN security_users.gender_id = 1 THEN institution_class_students.student_id END)) total_male_count
        ,COUNT(DISTINCT(CASE WHEN security_users.gender_id = 2 THEN institution_class_students.student_id END)) total_female_count
...
(
    SELECT institutions.id institution_id
        ,month_generator.date_info
        ,month_generator.start_date
        ,YEAR(month_generator.date_info) year_info
    FROM institutions
    INNER JOIN 
    (
        SELECT YEAR(m1) AS year_name
        ...
        FROM
        (
            SELECT dates_boundaries.start_date
                ,(dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH AS m1
                ,dates_boundaries.end_date
                ,DATE((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH + INTERVAL days.day DAY) AS generated_date
            FROM 
            (
                SELECT MIN(student_attendance_marked_records.date) start_date
            ...
            (
                ...
                WHERE calendar_events.institution_id = -1
                GROUP BY calendar_event_dates.date
            ) public_hol_info
            ON public_hol_info.public_hol = DATE((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH + INTERVAL days.day DAY)
            WHERE DATE((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH + INTERVAL days.day DAY) <= dates_boundaries.end_date
            AND MONTH(DATE((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH + INTERVAL days.day DAY)) = MONTH((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH)
            AND YEAR(DATE((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH + INTERVAL days.day DAY)) = YEAR((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH)
            AND DAYOFWEEK(DATE((dates_boundaries.start_date - INTERVAL DAYOFMONTH(dates_boundaries.start_date)-1 DAY) +INTERVAL m MONTH + INTERVAL days.day DAY)) BETWEEN working_days.first_day_of_week + 1 AND working_days.first_day_of_week + days_per_week
    ...
    AND private_holidays.date_info = month_generator.date_info
    WHERE private_holidays.calendar_event_id IS NULL
    GROUP BY month_generator.date_info
        ,institutions.id
) dates_generator
ON dates_generator.institution_id = main_query.institution_id
AND dates_generator.date_info BETWEEN main_query.start_date AND main_query.end_date
INNER JOIN 
(
    SELECT student_mark_type_status_grades.education_grade_id
        ...
        ,IFNULL(student_attendance_per_day_periods.period, 1) period_id
        ,IFNULL(student_attendance_per_day_periods.name, '') period_name
        ,IFNULL(subjects_info.education_subject_id, 0) subject_id
        ,IFNULL(subjects_info.education_subject_name, '') subject_name
    ...
    LEFT JOIN student_attendance_per_day_periods
    ON student_attendance_per_day_periods.student_attendance_mark_type_id = student_attendance_mark_types.id
    LEFT JOIN
    (
        SELECT education_systems.academic_period_id
            ,education_grades_subjects.education_grade_id
            ,education_grades_subjects.education_subject_id
            ,education_subjects.name education_subject_name
        ...
        INNER JOIN education_grades
        ON education_grades.id = education_grades_subjects.education_grade_id
    ...
    ON subjects_info.academic_period_id = student_mark_type_statuses.academic_period_id
    AND subjects_info.education_grade_id = student_mark_type_status_grades.education_grade_id
    AND student_attendance_mark_types.attendance_per_day = 0
) attendance_marking_type
ON attendance_marking_type.academic_period_id = main_query.academic_period_id
AND attendance_marking_type.education_grade_id = main_query.education_grade_id
LEFT JOIN 
(
    SELECT attendance_details.institution_id
        ...
        ,class_counter.present_male_count
        ,class_counter.present_female_count
    FROM 
    (
        SELECT student_attendance_marked_records.institution_id
            ,student_attendance_marked_records.academic_period_id
    ...
    (
        SELECT institution_class_students.education_grade_id
            ...
            ,institution_class_students.academic_period_id
            ,institution_class_students.institution_class_id
            ,COUNT(DISTINCT(CASE WHEN security_users.gender_id = 1 THEN institution_class_students.student_id END)) present_male_count
            ,COUNT(DISTINCT(CASE WHEN security_users.gender_id = 2 THEN institution_class_students.student_id END)) present_female_count
        FROM institution_class_students
        INNER JOIN security_users
        ON security_users.id = institution_class_students.student_id
        INNER JOIN academic_periods
    ...
    ON class_counter.education_grade_id = attendance_details.education_grade_id
...
ON marked_attendance.institution_id = main_query.institution_id
AND marked_attendance.academic_period_id = main_query.academic_period_id
AND marked_attendance.institution_class_id = main_query.institution_class_id
AND marked_attendance.education_grade_id = main_query.education_grade_id
AND marked_attendance.date = dates_generator.date_info
AND marked_attendance.period = attendance_marking_type.period_id
...
(
    SELECT institution_student_absence_details.academic_period_id
        ...
        ,institution_student_absence_details.period
        ,institution_student_absence_details.subject_id
        ,COUNT(DISTINCT(CASE WHEN security_users.gender_id = 2 AND institution_student_absence_details.absence_type_id != 3 THEN institution_student_absence_details.student_id END)) absent_female_count
        ,COUNT(DISTINCT(CASE WHEN security_users.gender_id = 1 AND institution_student_absence_details.absence_type_id != 3 THEN institution_student_absence_details.student_id END)) absent_male_count
...
ON marked_absence.institution_id = main_query.institution_id
AND marked_absence.academic_period_id = main_query.academic_period_id
AND marked_absence.institution_class_id = main_query.institution_class_id
AND marked_absence.education_grade_id = main_query.education_grade_id
AND marked_absence.date = dates_generator.date_info
AND marked_absence.period = attendance_marking_type.period_id
AND marked_absence.subject_id = attendance_marking_type.subject_id
ORDER BY main_query.academic_period_name
    ,main_query.institution_name
    ,main_query.education_grade_name
    ,main_query.institution_class_name
