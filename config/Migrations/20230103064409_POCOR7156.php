...
class POCOR7156 extends AbstractMigration
{
    ...
    public function up()
    {
        ...
        //Create table `security_user_otps`
        $table = $this->table('security_user_otps', [
                'collation' => 'utf8mb4_unicode_ci',
                'comment' => 'This table will contain the otps of the security users when they do login by two factor authentication'
            ]);
        ...
        $this->execute("ALTER TABLE `security_user_otps` ADD INDEX(`security_user_id`)");
        
        //INSERT data `Two Factor Authentication` in config_items table
        $this->execute("INSERT INTO `config_items` (`id`, `name`, `code`, `type`, `label`, `value`, `value_selection`, `default_value`, `editable`, `visible`, `field_type`, `option_type`, `modified_user_id`, `modified`, `created_user_id`, `created`) VALUES (NULL, 'Two Factor Authentication (Email)', 'two_factor_authentication', 'Authentication', 'Two Factor Authentication', '1', '', '1', '1', '1', 'Dropdown', 'completeness', '1', '".date('Y-m-d H:i:s')."', '1', '".date('Y-m-d H:i:s')."')"); 
        //INSERT data in report_queries table for cron (Karl Provided)
        $this->execute("INSERT INTO `report_queries` (`name`, `query_sql`, `frequency`, `status`, `modified_user_id`, `modified`, `created_user_id`, `created`) VALUES ('clear_otp', 'DELETE FROM security_user_otps WHERE created < DATE_SUB(NOW(), INTERVAL `1` HOUR);', 'hour', 1, NULL, NULL, 1, '".date('Y-m-d H:i:s')."')");    
    }
    ...
    public function down()
    {
        ...
        $this->execute('DROP TABLE security_user_otps');
    }
}
