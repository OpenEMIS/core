...
use App\Models\Api5\AppraisalNumberAnswers;
...
class AppraisalNumberAnswersApiTest extends TestCase
{
    ...
    public function test_can_list_AppraisalNumberAnswers()
    {
        if (AppraisalNumberAnswers::count() === 0) {
            AppraisalNumberAnswers::factory()->count(1)->create();
        }

        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token}",
        ])->getJson('/api/v5/appraisal-number-answers');

        $response->assertStatus(200);
    }

    public function test_can_create_AppraisalNumberAnswers()
    {
        $record = AppraisalNumberAnswers::factory()->make();
        $data = $record->toArray();

        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token}",
        ])->postJson('/api/v5/appraisal-number-answers', $data);
