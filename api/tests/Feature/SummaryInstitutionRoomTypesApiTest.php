...
use App\Models\Api5\SummaryInstitutionRoomTypes;
use App\Models\Api5\SecurityUsers as TestSecurityUser;
use Tymon\JWTAuth\Facades\JWTAuth;
use Carbon\Carbon;

class SummaryInstitutionRoomTypesApiTest extends TestCase
{
    ...
    public function test_can_list_SummaryInstitutionRoomTypes()
    {
        if (SummaryInstitutionRoomTypes::count() === 0) {
            SummaryInstitutionRoomTypes::factory()->count(1)->create();
        }

        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token}",
        ])->getJson('/api/v5/summary-institution-room-types');

        $response->assertStatus(200);
    }

    public function test_can_create_SummaryInstitutionRoomTypes()
    {
        $record = SummaryInstitutionRoomTypes::factory()->make();
        $data = $record->toArray();

        $response = $this->withHeaders([
            'Authorization' => "Bearer {$this->token}",
        ])->postJson('/api/v5/summary-institution-room-types', $data);
