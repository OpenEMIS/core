<?php

namespace Database\Factories;

use App\Models\TrainingCourses;
use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\Log;

class TrainingCoursesFactory extends Factory
{
    protected $model = TrainingCourses::class;

    public function definition(): array
    {


        return [
    'id' => $this->model::max('id') + 1,
    'code' => $this->faker->lexify(str_repeat("?", 60)),
    'name' => $this->faker->lexify(str_repeat("?", 250)),
    'description' => $this->faker->text(50),
    'objective' => $this->faker->text(50),
    'credit_hours' => $this->faker->numberBetween(1, 1000),
    'duration' => $this->faker->numberBetween(1, 1000),
    'number_of_months' => $this->faker->numberBetween(1, 1000),
    'special_education_needs' => $this->faker->numberBetween(1, 1000),
    'file_name' => $this->faker->lexify(str_repeat("?", 250)),
    'file_content' => $this->faker->word(),
    'training_field_of_study_id' => \App\Models\TrainingFieldOfStudies::inRandomOrder()->value('id') ?? 1,
    'training_course_type_id' => \App\Models\TrainingCourseTypes::inRandomOrder()->value('id') ?? 1,
    'training_course_category_id' => \App\Models\TrainingCourseCategories::inRandomOrder()->value('id') ?? 1,
    'training_mode_of_delivery_id' => \App\Models\TrainingModeDeliveries::inRandomOrder()->value('id') ?? 1,
    'training_requirement_id' => \App\Models\TrainingRequirements::inRandomOrder()->value('id') ?? 1,
    'training_level_id' => \App\Models\TrainingLevels::inRandomOrder()->value('id') ?? 1,
    'assignee_id' => \App\Models\SecurityUsers::inRandomOrder()->value('id') ?? 1,
    'status_id' => \App\Models\WorkflowSteps::inRandomOrder()->value('id') ?? 1,
    'modified_user_id' => $this->faker->numberBetween(1, 1000),
    'modified' => \Carbon\Carbon::now()->format("Y-m-d H:i:s"),
    'created_user_id' => $this->faker->numberBetween(1, 1000),
    'created' => \Carbon\Carbon::now()->format("Y-m-d H:i:s"),
];
    }
}
