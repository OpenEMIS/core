<?php

namespace Database\Factories;

use App\Models\UserAttachments;
use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\Log;

class UserAttachmentsFactory extends Factory
{
    protected $model = UserAttachments::class;

    public function definition(): array
    {


        return [
    'id' => $this->model::max('id') + 1,
    'student_attachment_type_id' => \App\Models\StudentAttachmentTypes::inRandomOrder()->value('id') ?? 1,
    'staff_attachment_type_id' => \App\Models\StaffAttachmentTypes::inRandomOrder()->value('id') ?? 1,
    'name' => $this->faker->lexify(str_repeat("?", 250)),
    'description' => $this->faker->text(50),
    'file_name' => $this->faker->lexify(str_repeat("?", 250)),
    'file_content' => $this->faker->word(),
    'date_on_file' => \Carbon\Carbon::now()->format("Y-m-d"),
    'security_user_id' => \App\Models\SecurityUsers::inRandomOrder()->value('id') ?? 1,
    'modified_user_id' => $this->faker->numberBetween(1, 1000),
    'modified' => \Carbon\Carbon::now()->format("Y-m-d H:i:s"),
    'created_user_id' => $this->faker->numberBetween(1, 1000),
    'created' => \Carbon\Carbon::now()->format("Y-m-d H:i:s"),
];
    }
}
