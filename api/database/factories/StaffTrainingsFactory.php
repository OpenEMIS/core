<?php

namespace Database\Factories;

use App\Models\StaffTrainings;
use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\Log;

class StaffTrainingsFactory extends Factory
{
    protected $model = StaffTrainings::class;

    public function definition(): array
    {


        return [
    'id' => $this->model::max('id') + 1,
    'code' => $this->faker->lexify(str_repeat("?", 60)),
    'name' => $this->faker->lexify(str_repeat("?", 100)),
    'description' => $this->faker->text(50),
    'credit_hours' => $this->faker->numberBetween(1, 1000),
    'file_name' => $this->faker->lexify(str_repeat("?", 250)),
    'file_content' => $this->faker->word(),
    'completed_date' => \Carbon\Carbon::now()->format("Y-m-d"),
    'staff_id' => \App\Models\SecurityUsers::inRandomOrder()->value('id') ?? \App\Models\SecurityUsers::factory()->create()->id,
    'staff_training_category_id' => \App\Models\StaffTrainingCategories::inRandomOrder()->value('id') ?? \App\Models\StaffTrainingCategories::factory()->create()->id,
    'training_field_of_study_id' => \App\Models\TrainingFieldOfStudies::inRandomOrder()->value('id') ?? \App\Models\TrainingFieldOfStudies::factory()->create()->id,
    'modified_user_id' => $this->faker->numberBetween(1, 2),
    'modified' => \Carbon\Carbon::now()->format("Y-m-d H:i:s"),
    'created_user_id' => $this->faker->numberBetween(1, 2),
    'created' => \Carbon\Carbon::now()->format("Y-m-d H:i:s"),
];
    }
}
