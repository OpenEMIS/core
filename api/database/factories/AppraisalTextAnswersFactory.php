...
class AppraisalTextAnswersFactory extends Factory
{
    ...
    public function definition(): array
    {
        $attempts = 0;
do {
    $appraisal_form_id = $this->faker->randomElement(\App\Models\AppraisalTextAnswers::pluck('appraisal_form_id')->toArray()) ?? 1;
            $appraisal_criteria_id = $this->faker->randomElement(\App\Models\AppraisalTextAnswers::pluck('appraisal_criteria_id')->toArray()) ?? 1;
            $institution_staff_appraisal_id = $this->faker->randomElement(\App\Models\AppraisalTextAnswers::pluck('institution_staff_appraisal_id')->toArray()) ?? 1;
    $exists = AppraisalTextAnswers::where('appraisal_form_id', $appraisal_form_id)
